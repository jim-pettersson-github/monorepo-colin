...
test('choose lit back-wall buttons and visibly close/reopen doors around Colin', async ({ page }, testInfo) => {
  ...
  await expect.poll(async () => (await lift()).queue).toEqual([2]);
  await page.getByRole('button', { name: 'Stå i dörren', exact: true }).click();
  await expect.poll(async () => (await savedPlayer(page)).depth).toBe(0);
  await page.getByRole('button', { name: 'Stäng dörrarna', exact: true }).click();
  await expect.poll(async () => (await lift()).landing.open, { timeout: 4000, intervals: [100] }).toBeLessThan(0.9);
  await expect.poll(async () => (await lift()).landing.open, { timeout: 4000, intervals: [100] }).toBe(1);
  ...
  await expect.poll(async () => (await lift()).destination, { timeout: 12000 }).toBe(2);
